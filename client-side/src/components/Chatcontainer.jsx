...
import formatMesTime from '../lib/utils';

const Chatcontainer = ({ selectedUser, setselectedUser }) => {
  const scrolEnd = useRef(null);

  useEffect(() => {
    if (scrolEnd.current) {
      scrolEnd.current.scrollIntoView({ behavior: 'smooth' });
    }
  }, [messagesDummyData.length]); // Ensures scroll on new messages
  ...
  return selectedUser ? (
        ...
        {messagesDummyData.map((msg, ind) => (
          ...
          >
            ...
            <div className='text-center text-xs'>
              ...
              />
              <p className='text-gray-500'>{formatMesTime(msg.createdAt)}</p>
            </div>
          </div>
        ))}
    ...
    // Fallback screen when no user is selected
